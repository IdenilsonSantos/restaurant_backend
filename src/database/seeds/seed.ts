...
async function seed() {
  ...
  const [, owner, maria, carlos, driverUser1, driverUser2] = await userRepo.save([
    userRepo.create({ name: 'Admin', email: 'admin@delivery.com', passwordHash: await bcrypt.hash(adminPassword, 10), phone: '11900000000', role: UserRole.ADMIN }),
    userRepo.create({ name: 'João Restaurante', email: 'owner@delivery.com', passwordHash: await bcrypt.hash(ownerPassword, 10), phone: '11911111111', role: UserRole.RESTAURANT_OWNER }),
    userRepo.create({ name: 'Maria Cliente', email: 'maria@delivery.com', passwordHash: await bcrypt.hash(customerPassword, 10), phone: '11922222222', role: UserRole.CUSTOMER }),
    userRepo.create({ name: 'Carlos Cliente', email: 'carlos@delivery.com', passwordHash: await bcrypt.hash(customerPassword, 10), phone: '11933333333', role: UserRole.CUSTOMER }),
    userRepo.create({ name: 'Ana Entregadora', email: 'ana@delivery.com', passwordHash: await bcrypt.hash(driverPassword, 10), phone: '11944444444', role: UserRole.DRIVER }),
    userRepo.create({ name: 'Pedro Entregador', email: 'pedro@delivery.com', passwordHash: await bcrypt.hash(driverPassword, 10), phone: '11955555555', role: UserRole.DRIVER }),
  ]);
  ...
  const [burger, pizza] = await restaurantRepo.save([
    restaurantRepo.create({ ownerId: owner.id, name: 'Burger House', description: 'Os melhores hambúrgueres artesanais da cidade', address: 'Av. Paulista, 1000 - São Paulo, SP', latitude: -23.5631, longitude: -46.6544, isOpen: true }),
    restaurantRepo.create({ ownerId: owner.id, name: 'Pizza Express', description: 'Pizzas no forno a lenha, entrega em 30 minutos', address: 'Rua Augusta, 500 - São Paulo, SP', latitude: -23.5551, longitude: -46.6600, isOpen: true }),
  ]);
  console.log('Restaurants seeded');

  // ── Products ───────────────────────────────────────────────────────────────

  const [xBurguer, , , batata, , margherita, calabresa, quatroQueijos, , refri] = await productRepo.save([
  ...
  const [ana] = await driverRepo.save([
    driverRepo.create({ userId: driverUser1.id, vehicleType: 'moto', licensePlate: 'ABC-1234', rating: 4.8, isAvailable: false, currentLatitude: -23.5605, currentLongitude: -46.6566 }),
    driverRepo.create({ userId: driverUser2.id, vehicleType: 'bicicleta', licensePlate: 'XYZ-5678', rating: 4.5, isAvailable: true, currentLatitude: -23.5580, currentLongitude: -46.6590 }),
  ]);
  ...
  await itemRepo.save([
    // Pedido 1
    itemRepo.create({ orderId: order1.id, productId: xBurguer.id, productName: xBurguer.name, productPrice: xBurguer.price, quantity: 2, subtotal: xBurguer.price * 2 }),
    itemRepo.create({ orderId: order1.id, productId: batata.id, productName: batata.name, productPrice: batata.price, quantity: 1, subtotal: batata.price * 1 }),
    // Pedido 2
    itemRepo.create({ orderId: order2.id, productId: margherita.id, productName: margherita.name, productPrice: margherita.price, quantity: 1, subtotal: margherita.price * 1 }),
    itemRepo.create({ orderId: order2.id, productId: refri.id, productName: refri.name, productPrice: refri.price, quantity: 2, subtotal: refri.price * 2 }),
    // Pedido 3
    itemRepo.create({ orderId: order3.id, productId: calabresa.id, productName: calabresa.name, productPrice: calabresa.price, quantity: 1, subtotal: calabresa.price * 1 }),
    itemRepo.create({ orderId: order3.id, productId: quatroQueijos.id, productName: quatroQueijos.name, productPrice: quatroQueijos.price, quantity: 1, subtotal: quatroQueijos.price * 1 }),
  ]);
  ...
  await deliveryRepo.save([
    ...
    deliveryRepo.create({ orderId: order1.id, driverId: ana.id, status: DeliveryStatus.DELIVERED, pickedUpAt: pickedUp, deliveredAt: delivered }),
    // Pedido 2 — aguardando entregador (em preparo ainda)
    deliveryRepo.create({ orderId: order2.id, driverId: null, status: DeliveryStatus.WAITING, pickedUpAt: null, deliveredAt: null }),
  ]);
